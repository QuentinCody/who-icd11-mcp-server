...
export const sqlTools: ToolEntry[] = [
	{
		...
		schema: {
			query: z
				.string()
				.describe(
					"SQL SELECT query to execute. Only SELECT, PRAGMA, and EXPLAIN statements are allowed.",
				),
			params: z
				.array(sqlParam)
				.optional()
		...
		handler: async (input, ctx) => {
			const { query, params } = input as {
				query: string;
				params?: (string | number | boolean | null)[];
			};
			if (!isReadOnly(query)) {
				throw new Error(
					"sql_query only allows SELECT, PRAGMA, and EXPLAIN statements. Use sql_exec for DDL/DML.",
				);
			}
	...
	},
	{
		name: "sql_exec",
		description:
			"Execute DDL/DML statements (CREATE TABLE, INSERT, UPDATE, DELETE).",
		schema: {
			query: z
				.string()
				.describe(
	...
	},
	{
		name: "sql_exec_batch",
		description:
			"Execute multiple DDL/DML statements in a single call. Much faster than calling sql_exec in a loop.",
		schema: {
			statements: z
				.array(
					z.object({
				...
				.describe(
					"Array of statements to execute sequentially in a single round-trip.",
				),
		},
		handler: async (input, ctx) => {
			const { statements } = input as {
				statements: {
					query: string;
					params?: (string | number | boolean | null)[];
				}[];
